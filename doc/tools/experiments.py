...
class Distributions(object):
    ...
    def __init__(self, dataset, previous_experiment=None, create_experiment=True, class_used=None, cmap='Blues', verbosity=1, debug=False, random_state=47):
    ...
    def plot(self, df=None):
        ...
        axes[5].imshow(self.f_2[:,None].repeat(2, axis=1), cmap=self.cmap, origin='lower', extent=[-.5, .5, -2.5, 2.5]);axes[5].set_title("A)\nf(X_2|Z_1=0)")
        axes[6].imshow(self.f_2_marginal[:,None].repeat(2, axis=1), cmap=self.cmap, origin='lower', extent=[-.5, .5, -2.5, 2.5]);axes[6].set_title("B)\nf(X_2|Z_2=1)")
        axes[7].imshow(self.f, cmap=self.cmap, origin='lower', extent=[-2.5, 2.5, -2.5, 2.5]);axes[7].set_title("C)\nf(X_1, X_2|Z_1=1, Z_2=1)")
        axes[8].imshow(self.f_1_marginal[None, :].repeat(2, axis=0), cmap=self.cmap, origin='lower', extent=[-2.5, 2.5, -.5, .5]);axes[8].set_title("D)\nf(X_1|Z_1=1)")
        axes[9].imshow(self.f_1[None, :].repeat(2, axis=0), cmap=self.cmap, origin='lower', extent=[-2.5, 2.5, -.5, .5]);axes[9].set_title("E)\nf(X_1|Z_2=0)")
        
        axes[10].imshow(self.f_z1_bar[:,None].repeat(2, axis=1), cmap=self.cmap, origin='lower', extent=[-.5, .5, -2.5, 2.5]);axes[10].set_title("F)\nf(Z_1=0|X_2)")
        axes[11].imshow(self.f_z1[:,None].repeat(2, axis=1), cmap=self.cmap, origin='lower', extent=[-.5, .5, -2.5, 2.5]);axes[11].set_title("G)\nf(Z_1=1|X_2)")
        axes[13].imshow(self.f_z2[None, :].repeat(2, axis=0), cmap=self.cmap, origin='lower', extent=[-2.5, 2.5, -.5, .5]);axes[13].set_title("H)\nf(Z_2=1|X_1)")
        axes[14].imshow(self.f_z2_bar[None, :].repeat(2, axis=0), cmap=self.cmap, origin='lower', extent=[-2.5, 2.5, -.5, .5]);axes[14].set_title("I)\nf(Z_2=0|X_1)")


        # Plot first the dataset that
        axes[1 if df is not None else 2] = self.dataset.plot(ax=axes[1 if df is not None else 2])

        axes[12].text(0.5,0.5, "P(Z_1=0, Z_2=0)={:.3f}%".format(100*self.f_0), size=18, ha="center", transform=axes[12].transAxes)
        ...
        if df is not None:
            ...
            if self.class_used == 1:
                # plot on the A) plot the sample having only X2 
                axes[5].scatter([0]*len(df.query(" `Z1`==0 and  `Z2`==1 and `True Positive`==1")), 
                                df.query(" `Z1`==0 and  `Z2`==1 and `True Positive`==1")['X2'], 
                                color='b', alpha=alpha, label="TP (n={})".format(len(df.query(" `Z1`==0 and  `Z2`==1 and `True Positive`==1"))))
                axes[5].scatter([0]*len(df.query(" `Z1`==0 and  `Z2`==1 and `False Positive`==1")), 
                                df.query(" `Z1`==0 and  `Z2`==1 and `False Positive`==1")['X2'], 
                            color='g', alpha=alpha, label="FP (n={})".format(len(df.query(" `Z1`==0 and  `Z2`==1 and `False Positive`==1"))))
                axes[5].scatter([0]*len(df.query(" `Z1`==0 and  `Z2`==1 and `False Negative`==1")),  
                                df.query(" `Z1`==0 and  `Z2`==1 and `False Negative`==1")['X2'], 
                                color='r', alpha=alpha, label="FN (n={})".format(len(df.query(" `Z1`==0 and  `Z2`==1 and `False Negative`==1"))))

                axes[7].scatter(df.query(" `Z1`==1 and  `Z2`==1 and `True Positive`==1")['X1'], 
                                df.query(" `Z1`==1 and  `Z2`==1 and `True Positive`==1")['X2'], 
                                color='b', alpha=alpha, label="TP (n={})".format(len(df.query(" `Z1`==1 and  `Z2`==1 and `True Positive`==1"))))
                axes[7].scatter(df.query(" `Z1`==1 and  `Z2`==1 and `False Positive`==1")['X1'], 
                                df.query(" `Z1`==1 and  `Z2`==1 and `False Positive`==1")['X2'], 
                            color='g', alpha=alpha, label="FP (n={})".format(len(df.query(" `Z1`==1 and  `Z2`==1 and `False Positive`==1"))))
                axes[7].scatter(df.query(" `Z1`==1 and  `Z2`==1 and `False Negative`==1")['X1'],  
                                df.query(" `Z1`==1 and  `Z2`==1 and `False Negative`==1")['X2'], 
                                color='r', alpha=alpha, label="FN (n={})".format(len(df.query(" `Z1`==1 and  `Z2`==1 and `False Negative`==1"))))

                axes[9].scatter(df.query(" `Z1`==1 and  `Z2`==0 and `True Positive`==1")['X1'],
                                [0]*len(df.query(" `Z1`==1 and  `Z2`==0 and `True Positive`==1")),  
                                color='b', alpha=alpha, label="TP (n={})".format(len(df.query(" `Z1`==1 and  `Z2`==0 and `True Positive`==1"))))
                axes[9].scatter(df.query(" `Z1`==1 and  `Z2`==0 and `False Positive`==1")['X1'], 
                                [0]*len(df.query(" `Z1`==1 and  `Z2`==0 and `False Positive`==1")),
                                color='g', alpha=alpha, label="FP (n={})".format(len(df.query(" `Z1`==1 and  `Z2`==0 and `False Positive`==1"))))
                axes[9].scatter(df.query(" `Z1`==1 and  `Z2`==0 and `False Negative`==1")['X1'], 
                                [0]*len(df.query(" `Z1`==1 and  `Z2`==0 and `False Negative`==1")),  
                                color='r', alpha=alpha, label="FN (n={})".format(len(df.query(" `Z1`==1 and  `Z2`==0 and `False Negative`==1"))))

            elif self.class_used == 0:
                # plot on the A) plot the sample having only X2 
                axes[5].scatter([0]*len(df.query(" `Z1`==0 and  `Z2`==1 and `True Negative`==1")), 
                ...
                axes[7].scatter(df.query(" `Z1`==1 and  `Z2`==1 and `True Negative`==1")['X1'], 
                ...
                axes[9].scatter(df.query(" `Z1`==1 and  `Z2`==0 and `True Negative`==1")['X1'],
