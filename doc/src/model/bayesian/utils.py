...
def estimate_pdf(X=None, approach='multi_distributions', imputation_method='multi_dimensional_weighting', resolution=20, bandwidth=None):
    ...
    if approach=='single_distribution':

        if imputation_method=='multi_dimensional_weighting':
            # See documentation
        ...
        if imputation_method in ['mice', 'knn', 'median', 'mean', 'naive']:
            from .stats import kernel_based_pdf_estimation
            for i in range(H):
                for j in range(W):
                    x = xygrid[0][i,j]
    ...
    elif approach=='multi_distributions':

        # TODO: should be remoded
        if imputation_method=='missing':
            # See documentation
